...
def find_bounding_box_mask(img_path, x_gt, y_gt, threshold=0.5):
  ...
  masks, boxes, pred_cls = get_prediction(img_path, threshold) #pred mask, box and class
  #img = cv2.imread(img_path)
  #img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
...
def frames_pID(pID, start_frame, output_path, frames_path):

  data_tmp = data[data['pID'] == pID]
  data_tmp = data_tmp[(data_tmp['frame'] >= start_frame) & (data_tmp['frame'] < 350)]
  ...
  for idx, f in enumerate(data_tmp['frame']): #data_tmp['frame'].iloc[0], data_tmp['frame'].iloc[-1]
    ...
    if(dist < 40):
      dict_masks_bb[f] = { 'id_annotation': str(idx).zfill(5), 'dist' : dist, 'mask': mask, 'bb': bb, 'x_GT': curr_x, 'y_GT': curr_y}
    


  return data_tmp, dict_masks_bb #return of the dataset with the frames having onli the pID selected
...
if __name__ == '__main__':
    ...
    if(MASK_RCNN):
        ...
        for key in dict_masks_bb.keys():
            ...
            final_img[y_sup:y_inf,x_sup:x_inf, :] = img_output[y_sup:y_inf,x_sup:x_inf, :]
            imageio.imwrite(OUTPUT_DIR+'Annotations/pID'+str(PID)+'/'+dict_masks_bb[key]['id_annotation']+'.jpg',\
